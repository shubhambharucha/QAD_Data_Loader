...
FIELD_RULES = {
    ...
    "Supplier Code":   {"type": "character", "max_len": 8},
    "Supplier Item":   {"type": "character", "max_len": 30},
    "Unit of Measure": {"type": "character", "max_len": 2},
    "Quote Price":     {"type": "decimal",   "min": 0},
...
def _check_date(value):
    ...
    if isinstance(value, datetime):
        return None
    try:
        float(value)  # Excel serial date — openpyxl may pass these through
        return None
    except (ValueError, TypeError):
        pass
    ...
    for row_idx, row in enumerate(ws.iter_rows(min_row=2), start=2):
        ...
        for col_name, rule in FIELD_RULES.items():
            ...
            if rule["type"] == "character":
                ...
                if is_empty:
                    err = "empty"
                elif rule.get("max_len") and len(str_val) > rule["max_len"]:
                    err = f"max {rule['max_len']} chars (got {len(str_val)})"
                else:
                    err = None
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        if not xlsx_files:
            print("ERROR: No .xlsx file found. Usage: python validate_supplier_item.py <file.xlsx>")
            sys.exit(1)
