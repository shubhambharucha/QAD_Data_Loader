...
UPLOAD_URL = "https://cat5-devl.adaptive.qad.com/clouderp/api/erp/supplierV2s?viewUri=urn:be:com.qad.base.supplier.ISupplierV2"
...
def run(file_path):
    ...
    for row_idx, row in enumerate(ws.iter_rows(min_row=2), start=2):
        ...
        if str(row_data.get("Status", "")).strip().upper() == "DONE":
            supp = str(row_data.get("Supplier", "")).strip()
            print(f"  ⏭  {supp or f'row {row_idx}'} — already DONE")
            skip_count += 1
            continue

        supplier_code       = str(row_data.get("Supplier", "")).strip()
        shared_set          = str(row_data.get("Shared Set", "QMI-SUP")).strip()
        business_relation   = str(row_data.get("Business Relation", "")).strip()
        is_active           = str(row_data.get("Active", "yes")).strip().lower() == "yes"
        currency            = str(row_data.get("Currency", "")).strip()
        credit_terms        = str(row_data.get("Credit Terms", "")).strip()
        invoice_status      = str(row_data.get("Invoice Status", "")).strip()
        address_search_name = str(row_data.get("Address Search Name", "")).strip()
        city                = str(row_data.get("City", "")).strip()
        tax_zone            = str(row_data.get("Tax Zone", "USA-NJ")).strip()
        address_1           = str(row_data.get("Address 1", "")).strip()

        missing = []
        if not supplier_code:     missing.append("Supplier")
        if not business_relation: missing.append("Business Relation")
        if not shared_set:        missing.append("Shared Set")
        ...
        payload = {
            ...
            "supplierV2s": [{
                ...
                "businessRelationCode":             business_relation,
                "sharedSetCode":                    shared_set,
                "isActive":                         is_active,
                "currencyCode":                     currency,
                "creditTermsCode":                  credit_terms,
                "invoiceStatusCode":                invoice_status,
                "addressSearchName":                address_search_name,
                "city":                             city,
                "street1":                          address_1,
                "taxZone":                          tax_zone,
                "supplierID":                       0,
                ...
                "invoiceControlGLProfileCode":      "APcontrol3rdparty",
                "creditNoteControlGLProfileCode":   "APcontrol3rdparty",
                "prePaymentControlGLProfileCode":   "APcontrol3rdparty",
                "purchaseAccountGLProfileCode":     "Purchase",
                "dataOperation":                    "",
                "concurrencyHash":                  "",
                "disallowedActions":                "",
        ...
        while retry:
            try:
                ...
                if response.status_code == 200 and resp_json.get("submitResult", {}).get("success"):
                    ...
                    for e in errors:
                        ...
                        if "already exists" in msg.lower():
                            msgs.append(f"Duplicate — Supplier '{supplier_code}' already exists")
                    ...
                    print(f"  ✘  Supplier: {supplier_code} — {error_msg}")
                    fail_count += 1
                ...
                print(f"  ✘  Supplier: {supplier_code} — Connection error: {e}")
                fail_count += 1
                retry = False
...
if __name__ == "__main__":
    ...
    folder = os.path.join(base, "..", "Supplier")
    ...
    if not files:
        print("⚠  No .xlsx files found in Supplier/")
        sys.exit(0)
