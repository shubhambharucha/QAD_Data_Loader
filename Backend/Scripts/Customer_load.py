...
UPLOAD_URL = "https://cat5-devl.adaptive.qad.com/clouderp/api/erp/customerV2s?viewUri=urn:be:com.qad.base.customer.ICustomerV2"
...
def run(file_path):
    ...
    for row_idx, row in enumerate(ws.iter_rows(min_row=2), start=2):
        ...
        if str(row_data.get("Status", "")).strip().upper() == "DONE":
            cust = str(row_data.get("Customer", "")).strip()
            print(f"  ⏭  {cust or f'row {row_idx}'} — already DONE")
            skip_count += 1
            continue

        customer_code       = str(row_data.get("Customer", "")).strip()
        shared_set          = str(row_data.get("Shared Set", "")).strip()
        ...
        tax_zone            = str(row_data.get("Tax Zone", "USA-TX")).strip()
        address_1           = str(row_data.get("Address 1", "")).strip()

        missing = []
        if not customer_code:     missing.append("Customer")
        ...
        while retry:
            try:
                ...
                if response.status_code == 200 and resp_json.get("submitResult", {}).get("success"):
                    ...
                    for e in errors:
                        ...
                        if "already exists" in msg.lower():
                            msgs.append(f"Duplicate — Customer '{customer_code}' already exists")
                    ...
                    print(f"  ✘  Customer: {customer_code} — {error_msg}")
                    fail_count += 1
                ...
                print(f"  ✘  Customer: {customer_code} — Connection error: {e}")
                fail_count += 1
                retry = False
...
if __name__ == "__main__":
    ...
    folder = os.path.join(base, "..", "Customer")
    ...
    if not files:
        print("⚠  No .xlsx files found in Customer/")
        sys.exit(0)
