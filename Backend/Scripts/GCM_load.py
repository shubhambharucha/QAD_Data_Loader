...
UPLOAD_URL = "https://cat5-devl.adaptive.qad.com/clouderp/api/erp/generalizedCodes?viewUri=urn:be:com.qad.base.codes.IGeneralizedCode"
...
def run(file_path):
    ...
    for row_idx, row in enumerate(ws.iter_rows(min_row=2), start=2):
        ...
        if str(row_data.get("Status", "")).strip().upper() == "DONE":
            val = str(row_data.get("Value", "")).strip()
            print(f"  ⏭  {val or f'row {row_idx}'} — already DONE")
            skip_count += 1
            continue

        domain_code = str(row_data.get("Domain", "")).strip()
        field_name  = str(row_data.get("Field Name", "")).strip()
        code_value  = str(row_data.get("Value", "")).strip()
        comment     = str(row_data.get("Comment", "")).strip()
        code_group  = str(row_data.get("Group", "")).strip()

        missing = []
        if not domain_code: missing.append("Domain")
        if not field_name:  missing.append("Field Name")
        if not code_value:  missing.append("Value")
        ...
        while retry:
            try:
                ...
                if response.status_code == 200 and resp_json.get("submitResult", {}).get("success"):
                    ...
                    print(f"  ✔  GCM: {domain_code} | {field_name} | {code_value}")
                    success_count += 1
                else:
                    ...
                    for e in errors:
                        ...
                        if "already exists" in msg.lower():
                            msgs.append(f"Duplicate — '{field_name}/{code_value}' already exists")
                    ...
                    print(f"  ✘  GCM: {domain_code} | {field_name} | {code_value} — {error_msg}")
                    fail_count += 1
                ...
                print(f"  ✘  GCM: {field_name} — Connection error: {e}")
                fail_count += 1
                retry = False
...
if __name__ == "__main__":
    ...
    if not files:
        print("⚠  No .xlsx files found in GCM/")
        sys.exit(0)
